...
import { addPost, posts, likes, dislikes } from '../../redux/action';
          ...
          <div className="col-6">
            ...
            <div className="row">
              {props.posts.map(post => {
                ...
                const yaTieneLike = likeId.includes(props.user.id)
                  ...
                  <div className="card posts ">
                    ...
                    <div className="card-body PiePag  ">
                      ...
                      <div className='d-flex align-items-center  '   >
                        {console.log(post.likes.map(like=>like.user_id))}
                        {yaTieneLike ?
                        
                          <div className='d-flex align-items-center m-0'  >
                            <button type='button' className="desaparecer" onClick={yaTieneLike ?() => dislikes(post.id, post): () =>likes(post.id, post) } ><HeartFilled className='like' /> </button><p className='m-0 ml-2 '  > <p className='     d-flex align-items-center m-0'>{(post.likes).length}</p></p> 
                          </div>
                          :
                          <div className='d-flex align-items-center m-0'   >
                            <button className="desaparecer" type='button' onClick={yaTieneLike ?() => dislikes(post.id, post): () =>likes(post.id, post)}><HeartOutlined className='like' /> </button> <p className='m-0 ml-2 '  > <p className='d-flex align-items-center m-0'>{(post.likes).length}</p></p>
                          </div>
                        }
